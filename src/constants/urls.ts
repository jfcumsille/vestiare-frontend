export const DOCS = 'https://docs.fintoc.com';
export const CONTACT = 'https://fintoc.com/contact';
export const FINTOC_HOME = 'https://fintoc.com';

export const DOCS_LINKS = 'https://docs.fintoc.com/reference/links';
export const DOCS_LINK_TOKEN = 'https://docs.fintoc.com/reference/links-retrieve';
export const DOCS_SANDBOX = 'https://docs.fintoc.com/docs/sandbox-usernames-and-passwords';
export const DOCS_WEBHOOKS = 'https://docs.fintoc.com/docs/webhooks-walkthrough';
export const DOCS_API_KEYS = 'https://docs.fintoc.com/docs/api-keys';
export const DOCS_API_CHANGELOG = 'https://docs.fintoc.com/docs/api-changelog';
export const DOCS_EVENTS = 'https://docs.fintoc.com/docs/events-types';
export const DOCS_PAYMENTS = 'https://docs.fintoc.com/docs/payment-intents-walkthrough';
export const DOCS_PAYMENTS_LEARN_MORE = 'https://docs.fintoc.com/docs/payment-intents-walkthrough';
export const DOCS_PAYMENTS_LEARN_HOW = 'https://docs.fintoc.com/docs/payment-intents-integration-flow';
export const DOCS_PAYMENTS_ERROR_REASONS = 'https://docs.fintoc.com/reference/payment-intent-error-reason';
export const DOCS_METADATA = 'https://docs.fintoc.com/reference/metadata';
export const TERMS_AND_CONDITIONS = 'https://fintoc.com/legal/terminos-y-condiciones-para-desarrolladores#terminos-y-condiciones';
export const PRIVACY_POLICY = 'https://fintoc.com/legal/politicas-de-privacidad-para-usuarios-finales';

export const SALES_EMAIL = 'mailto:sales@fintoc.com';
